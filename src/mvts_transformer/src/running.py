...
class SupervisedRunner(BaseRunner):
    ...
    def inference(self):
        ...
        Args:
        X (Tensor): The input features, typically a tensor of shape (batch_size, features).
        padding_masks (Tensor): Masks indicating which elements in the batch should be ignored.

        Returns:
        Numpy: The predictions from the model.
        """
        # 모델을 평가 모드로 설정
        self.model = self.model.eval()
        ids = []
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.dataloader):
                ...
                targets = targets.to(self.device)
                padding_masks = padding_masks.to(self.device)  # 배치별 padding_masks도 디바이스로 이동

                predictions = self.model(X.to(self.device), padding_masks)  # 모델 예측 수행
                ids.append(IDs)
                res.append(predictions.cpu())  # 예측 결과를 CPU로 이동 후 리스트에 추가
        
        def extract_unique(ids, res):
            unique_results = {}  # 고유한 결과를 저장할 딕셔너리
            for id_list, result_list in zip(ids, res):  # ids와 res의 각 리스트를 순회
                for id_value, result in zip(id_list, result_list):  # 리스트 내의 각 id와 결과를 순회
                    if id_value not in unique_results:  # 현재 id가 딕셔너리에 없다면
                        unique_results[id_value] = result  # 결과를 딕셔너리에 추가

            # 결과를 id에 따라 정렬하여 반환
            sorted_ids = sorted(unique_results.keys())  # 키(고유 ID)를 기준으로 정렬
            sorted_results = [unique_results[id] for id in sorted_ids]  # 정렬된 키에 해당하는 결과 추출

            return sorted_ids, sorted_results  # 정렬된 id와 해당 결과 반환
        
        sorted_ids, sorted_res = extract_unique(ids, res)

        sorted_res_list = []
        for s in sorted_res:
            sorted_res_list.append(list(s))

        # 결과를 NumPy 배열로 변환
        sorted_ids_np = np.array(sorted_ids)  # ID 배열 변환
        sorted_res_np = np.array(sorted_res_list)  # 결과 배열 변환

        print("Sorted IDs:", sorted_ids_np)
        print("Corresponding Results:", sorted_res_np)

        np.save('sorted_ids_reg.npy', sorted_ids_np)
        np.save('sorted_res_reg.npy', sorted_res_np)

        # # 모든 배치의 결과를 하나의 텐서로 연결
        # res = torch.cat(res, dim=0)  # 결과를 연결 (차원 0을 따라)
        # return res.numpy()  # 최종 결과를 numpy 배열로 변환 후 반환